...
すなわち,図\ref{fig:propose}のように交渉開始時の受諾水準が\mathrm{L}であった場合,一定時間経過後受諾水準を\mathrm{L\pm \alpha},さらに一定時間経過後受諾水準を\mathrm{L\pm 2\alpha}と変化させる.以下,\mathrm{\alpha}を内側のパラメータ,\mathrm{\alpha}を使用する戦略を内側の戦略と記述する.
また,同時に交渉回数の増加に伴って受諾水準を変化させる.
すなわち,図\ref{fig:propose}のように交渉開始時の受諾水準が\mathrm{L}であった場合,2回目の交渉開始時の受諾水準を\mathrm{L\pm \beta},3回目の交渉開始時の受諾水準を\mathrm{L\pm 2\beta}と変化させる.以下,\mathrm{\beta}を外側のパラメータ,\mathrm{\beta}を使用する戦略を外側の戦略と記述する.
...
NotNotを除外した8種類の戦略を提案手法とし,これらの戦略をIAGOのサンプルエージェントであるPinocchioエージェントに適応させたエージェントを用いて第4章の予備実験および第5章の評価実験を行う.
